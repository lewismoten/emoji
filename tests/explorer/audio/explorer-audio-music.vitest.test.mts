...
describe("explorer-audio-music", () => {
  it("schedules theme songs and individual song events across rest and sustain cases", async () => {
    ...
    assert.equal(
      (lightSchedule.musicGain as unknown as FakeGain).connectedTo[0],
      masterGain,
    );
    ...
    assert.equal(
      (context.oscillators[0] as FakeOscillator).periodicWave != null,
      true,
    );
    ...
    assert.equal(
      (context.oscillators.at(-1) as FakeOscillator).periodicWave != null,
      true,
    );
    assert.equal(
    ...
    const fallbackOutput =
      fallbackReleaseContext.createGain() as unknown as GainNode;
    scheduleExplorerSongEventForTest(
    ...
    assert.equal(
      (sustainContext.oscillators[0] as FakeOscillator).frequency.calls.some(
        ([name, value]) =>
          name === "exponentialRampToValueAtTime" && value === 330,
      ),
      true,
    );
