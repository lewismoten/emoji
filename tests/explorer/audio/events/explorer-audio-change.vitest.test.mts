...
describe("explorer-audio-change", () => {
  it("reacts to delegated audio preference changes and interactive controls", async () => {
    ...
    const handler = buildExplorerAudioChange(dependencies as any);
    ...
    (dependencies as any).getInteractiveTarget = () => null;
    const interactiveMiss = new FakeHTMLElement();
    await handler(asEvent(interactiveMiss));
    assert.deepEqual(calls, []);

    (dependencies as any).getInteractiveTarget = () => {
