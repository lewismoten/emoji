...
import { FakeElement, installFakeDocument } from "../../controls/fake-dom.mjs";
import {
  createDialogHeading,
  createHeading,
  createTextBlock,
...
describe("dialog-control-helpers", () => {
  it("creates headings, text blocks, and pressed states", () => {
    ...
    try {
      ...
      assert.equal(dialogHeading.children.length, 2);
      ...
      setPressedState(toggleTarget, false);
      assert.deepEqual(toggleTarget.classList.toggled.at(-1), [
        "is-selected",
        false,
      ]);
      assert.equal(toggleTarget.attributes.get("aria-pressed"), "false");
    } finally {
      restore();
    }
  });
