...
describe("dialog-upgrade", () => {
  ...
  it("upgrades the emoji dialog and removes legacy elements", () => {
    ...
    assert.equal(
      exampleDialog.querySelector('[data-i18n="copiedDescription"]'),
      null,
    );
    ...
    const parentCountBefore =
      dialogControls.querySelectorAll(".emoji-parent").length;
    upgradeEmojiDialog({
    ...
    removeLegacyDialogElements();
    assert.equal(
      legacyDialog.querySelector('[data-i18n="copiedDescription"]'),
      null,
    );
  });
});
