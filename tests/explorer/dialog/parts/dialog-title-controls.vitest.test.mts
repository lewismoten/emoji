...
describe("dialog-title-controls", () => {
  it("creates, reuses, and positions favorite button controls", () => {
    ...
    assert.equal(
      ensureDialogTitleRow(dialogTitle as unknown as HTMLElement),
      titleRow,
    );
    ...
    assert.equal(
      (insertedTitleMain as unknown as FakeElement).children[0],
      titledHeading,
    );
    ...
    const ensuredFavorite = ensureFavoriteButton(
      dialogControls as unknown as HTMLElement,
    );
    ...
    (ensuredFavorite as unknown as FakeElement).dataset.i18nAriaLabel =
      "removeFavorite";
    (ensuredFavorite as unknown as FakeElement).setAttribute(
      "aria-label",
      "Remove favorite",
    );
    (ensuredFavorite as unknown as FakeElement).title = "Remove favorite";

    const reusedFavorite = ensureFavoriteButton(
      dialogControls as unknown as HTMLElement,
    );
