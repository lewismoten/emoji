...
try {
  ...
  exampleDialog.append(compositionSection, diagnosticSection, invitation, copyActions);
  ...
  updateEmojiComposition({
    applyPixelArtworkClass: (element, emojiKey) => {
      appliedPixelKey = emojiKey;
      element.classList.add("has-pixel-art");
    },
    applyStandalonePixelArtwork: (element, emojiKey) => {
      if (element && emojiKey) element.dataset.pixelEmojiKey = emojiKey;
    },
    ...
    item: { key: "womanTechnologist", shortName: "woman technologist", codePoints: "1F469 200D 1F4BB" },
    locale: "en",
  ...
  assert.match(renderingResult.textContent ?? "", /Pixel Emoji keeps the sequence together/);
  ...
  let renderingCalls: Array<[string, string]> = [];
  let compositionCalls: Array<[any, string]> = [];
  const dialogDisplay = renderEmojiDialog({
    ...
    updateRenderingDiagnostic: (emojiKey, value) => {
      renderingCalls.push([emojiKey, value]);
    },
    updateEmojiComposition: (item, value) => {
      compositionCalls.push([item, value]);
    },
    value: "🎁",
  });
