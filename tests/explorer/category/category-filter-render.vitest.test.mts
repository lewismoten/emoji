...
class FakeElement {
  ...
  setAttribute(name: string, value: string) {
    ...
    if (name === "role") {
      this.dataset.role = value;
    }
    if (name === "tabindex") {
      this.dataset.tabindex = value;
    }
  ...
  closest(selector: string) {
    ...
    while (current) {
      if (
        ...
        current.className.includes("filter-field")
      ) {
        return current;
      }
      if (
        selector === ".compact-group-choices" &&
        current.className.includes("compact-group-choices")
      ) {
        return current;
      }
      if (
        selector === ".compact-subgroup-choices" &&
        current.className.includes("compact-subgroup-choices")
      ) {
        return current;
      }
      if (
        selector === ".compact-sequence-choices" &&
        current.className.includes("compact-sequence-choices")
      ) {
        return current;
      }
      if (
...
const appendTriggerContents = (trigger: FakeElement) => {
  const emoji = new FakeElement("span");
  emoji.className = "filter-picker-emoji";
  const value = new FakeElement("span");
  value.className = "filter-picker-value";
  trigger.append(emoji, value);
};
...
describe("category-filter-render", () => {
  ...
  it("updates available categories and renders grouped controls", () => {
    ...
    compactSubGroupChoices.className =
      "compact-choices compact-subgroup-choices";
    ...
    expect([...availableCategoryKeys]).toEqual(["smile", "car"]);
    expect(availableGroups).toEqual(["Smileys & Emotion", "Travel & Places"]);
    expect(availableSubGroups).toEqual({
      "Smileys & Emotion": ["face-smiling"],
      "Travel & Places": ["transport-ground"],
    });
    expect(availableSequenceTypes).toEqual(["single", "zwj"]);
    expect(selectedGroup).toBe("");
    expect(selectedSequenceType).toBe("");
    ...
    expect(groupSelectorField.classList.has("has-choice-buttons")).toBe(true);
    expect(subGroupSelectorField.classList.has("has-choice-buttons")).toBe(true);
    expect(sequenceSelectorField.classList.has("has-choice-buttons")).toBe(true);
    expect(groupSelectorField.hidden).toBe(false);
    expect(subGroupSelectorField.hidden).toBe(false);
    expect(sequenceSelectorField.hidden).toBe(true);
    expect(groupSelector.value).toBe("Smileys & Emotion");
    expect(subGroupSelector.value).toBe("Smileys & Emotion::face-smiling");
    expect(sequenceSelector.value).toBe("single");
    expect(compactGroupLabel.textContent).toBe("shown:Smileys & Emotion");
    expect(compactSubGroupLabel.textContent).toBe("Face Smiling");
    expect(compactSequenceLabel.textContent).toBe("Single");
    expect(groupPickerValue.textContent).toBe("shown:Smileys & Emotion");
    expect(subGroupPickerValue.textContent).toBe("Face Smiling");
    expect(compactGroupChoices.children).toHaveLength(3);
    expect(compactSubGroupChoices.children).toHaveLength(2);
    expect(compactSequenceChoices.children).toHaveLength(3);
    ...
    renderer.renderCategoryFilters();
    expect(compactSequenceChoices.children[2]?.focused).toBe(true);
  });

  it("renders sequence mode with group and subgroup fields hidden", () => {
    Object.defineProperty(globalThis, "document", {
      configurable: true,
      value: documentStub,
    });
    ...
    state.subGroups.replace({ "Smileys & Emotion": ["face-smiling"] });
    ...
    const groupPickerTrigger = new FakeElement("button");
    const subGroupPickerTrigger = new FakeElement("button");
    appendTriggerContents(groupPickerTrigger);
    appendTriggerContents(subGroupPickerTrigger);
    ...
    renderer.renderCategoryFilters();
    expect([...availableCategoryKeys]).toEqual(["smile"]);
    expect(availableSequenceTypes).toEqual(["single"]);
    expect(groupSelectorField.hidden).toBe(true);
    expect(subGroupSelectorField.hidden).toBe(true);
    expect(sequenceSelectorField.hidden).toBe(false);
  });
});
