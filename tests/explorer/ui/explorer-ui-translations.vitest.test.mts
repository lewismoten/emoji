...
describe("explorer-ui-translations", () => {
  it("loads translations, updates document metadata, and handles install state", async () => {
    ...
    try {
      ...
      assert.equal((globalThis.document as any).title, "Emoji Explorer");
      ...
      assert.equal((globalThis.document as any).title, "مستكشف الإيموجي");
