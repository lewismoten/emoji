...
describe("version-filter-control", () => {
  it("creates and reuses sequence, version slider, and version mode controls", () => {
    ...
    try {
      ...
      assert.equal(
        selector.attributes.get("aria-labelledby"),
      ...
      const sharedVersionSelector = new FakeElement("select");
      sharedVersionSelector.parent = sharedField;
      sharedVersionSelector.closest = (query: string) =>
        query === ".filter-field" ? sharedField : null;
      const inlineRange = new FakeElement("input");
      inlineRange.parent = sharedField;
      inlineRange.closest = (query: string) =>
        query === ".compact-version" ? null : null;
      const existingToggle = new FakeElement("button");
      existingToggle.className = "version-mode-toggle";
      Object.defineProperty(globalThis, "document", {
        configurable: true,
        value: {
          head: new FakeElement("head"),
      ...
      const reusedToggle = ensureVersionModeToggleControl({
        document: globalThis.document,
        versionModeSelector: inlineModeSelector as any,
        versionRange: () => inlineRange as any,
        versionSelector: sharedVersionSelector as any,
      });
      assert.equal(reusedToggle, existingToggle);
      assert.equal(sharedField.classList.values.has("developer-only"), false);
    } finally {
      if (originalDocument) {
