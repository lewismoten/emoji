...
describe("filter-picker-control", () => {
  it("creates dialog, trigger, and compact choice controls", () => {
    ...
    try {
      ...
      assert.equal(
        compactChoice.getAttribute("aria-label"),
        "Smileys & Emotion",
      );
      ...
      assert.equal(
        compactChoice.children[1].textContent,
        "Smileys & Emotion",
      );
      ...
      const stylesheetIds = documentStub.head.children
        .map((child) => child.id)
        .sort();
