...
describe("pwa-on-panel-dialog-close", () => {
  it("replaces panel history state for recognized panel dialogs", () => {
  ...
  it("skips suppressed, applying, and not-ready close events", () => {
    const syncCalls: Array<
      [mode?: "replace" | "push", state?: Record<string, unknown>]
    > = [];
  ...
  it("handles array-like history state and unknown dialogs", () => {
    const replaceCalls: string[] = [];
    const syncCalls: Array<
      [mode?: "replace" | "push", state?: Record<string, unknown>]
    > = [];
    ...
    assert.deepEqual(replaceCalls.at(-1), "/demo?mode=retro#focus");
