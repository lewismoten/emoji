...
describe("search-language-lifecycle", () => {
  ...
  it("renders, loads, selects, and restores language state", async () => {
    ...
    expect(renderHarness.languageList.children).toHaveLength(2);
    ...
    expect(staleHarness.getSelectedSearchLocale()).toBe("");

    const setHarness = createLifecycleHarness({ nextLoadId: 4, currentLoadId: 4 });
    await setHarness.lifecycle.set("");
    expect(setHarness.getSelectedSearchLocale()).toBe("");
    expect(setHarness.getSearchAnnotations()).toEqual({});
    expect(setHarness.getSearchLabels()).toEqual({});
    expect(setHarness.getSearchSubgroupLabels()).toEqual({});
    expect(
    ...
    expect(setHarness.languageList.children.length).toBeGreaterThanOrEqual(1);
    ...
    expect(historyCalls[historyCalls.length - 1].href).toBe(
    ...
    expect(manifestCalls).toEqual(["locales/manifest.json", "locales/ar.json"]);
    expect(loadHarness.getSearchLocales()).toHaveLength(1);
    expect(loadHarness.getSelectedSearchLocale()).toBe("ar");
    expect(loadHarness.getSearchAnnotations()).toEqual({});
    expect(loadHarness.getSearchLabels()).toEqual({ greeting: "تحية" });
    expect(loadHarness.getSearchSubgroupLabels()).toEqual({ hand: "يد" });
    expect(loadHarness.languageList.children.length).toBeGreaterThanOrEqual(1);
    ...
    expect(
      invalidInitialHarness.calls.filter(
        (entry) => entry[0] === "setSelectedLocale",
      ),
    ).toEqual([]);
    ...
    expect(failedLoadHarness.languagePicker.disabled).toBe(true);
    expect(warnings[0]?.[0]).toBe("Search language packs unavailable");
    ...
    expect(
      ignoredPopHarness.calls.filter(
        (entry) => entry[0] === "setSelectedLocale",
      ),
    ).toEqual([]);
    ...
    expect(newspeakPopHarness.getSelectedSearchLocale()).toBe("en-x-newspeak");
    expect(
