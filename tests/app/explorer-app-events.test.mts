...
const sourceModuleSpecifier = "build/src/app/explorer-app-events.js";
...
try {
  ...
  const developerModeToggle = createEventTarget(),
    languagePicker = createEventTarget();
  ...
  const choiceOne = createEventTarget(),
    choiceTwo = createEventTarget();
  const themeChoice = createEventTarget(),
    installedQuery = createEventTarget();
  ...
  assert.equal(lifecycleCalls.includes("render-saved"), false);
