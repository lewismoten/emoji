...
describe("explorer-preferences", () => {
  it("hydrates state from stored preferences and persists updates", () => {
  ...
  it("falls back safely for bad json, blocked writes, and invalid arrays", () => {
    installWindow({
      localStorage: {
        getItem() {
          return "{bad json";
  ...
  it("handles missing storage and legacy developer-mode data", () => {
    installWindow({
      localStorage: {
        getItem() {
          return null;
