...
describe("buildExplorerBootstrapRuntimeOptions", () => {
  it("maps runtime bootstrap options to wrapper functions and direct references", () => {
    ...
    for (const key of [
      ...
      expect(runtimeOptions[key]).toBe(options[key]);
    }

    expect(runtimeOptions.activeFilters()).toBe("advancedFilters-value");
    expect(runtimeOptions.modeChoices()).toBeUndefined();
    ...
    expect(runtimeWithModes.modeChoices()).toBe("modeChoices-value");
