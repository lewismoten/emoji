...
describe("pixel-editor-transfer-skin-tone", () => {
  it("finds matching skin tone helpers across manifest and drafts", async () => {
    ...
    try {
      let result = await findSkinTonePasteHelper({
      ...
      result = await findSkinTonePasteHelper({
        artworkDrafts: () => new Map(),
      ...
      setFetch(
        async (input: string | URL | Request) =>
          ({
            ...
            headers: {
              get: () =>
                String(input).includes("helper.png")
                  ? "image/png"
                  : "text/plain",
            },
            blob: async () => ({ input }),
