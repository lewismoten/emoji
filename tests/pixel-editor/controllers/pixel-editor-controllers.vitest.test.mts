...
describe("pixel-editor-controllers", () => {
  it("builds and wires the controller composition root", async () => {
    ...
    const module = await import(
      pathToFileURL(path.join(tempDirectory, "pixel-editor-controllers.mjs"))
        .href
    );
    const draftStub = await import(
      pathToFileURL(path.join(tempDirectory, "pixel-editor-drafts-stub.mjs"))
        .href
    );
    const startupStub = await import(
      pathToFileURL(path.join(tempDirectory, "pixel-editor-startup-stub.mjs"))
        .href
    );
