...
describe("pixel-editor-startup", () => {
  it("wires the UI controls and handlers", () => {
    ...
    try {
      initializePixelEditorUi({
        ...
        paletteController: {
          selectPaletteColor: (button: unknown) =>
            calls.push(["palette", button]),
    ...
    assert.equal(
      calls.some(
        (entry) =>
          Array.isArray(entry) &&
          entry[0] === "transform-layer" &&
          entry[1] === "rotate-right",
      ),
      true,
    );
    ...
    assert.equal(calls.includes("pointerdown"), true);
    assert.equal(calls.includes("pointermove"), true);
    assert.equal(calls.includes("pointerup"), true);
    assert.equal(calls.includes("pointercancel"), true);
