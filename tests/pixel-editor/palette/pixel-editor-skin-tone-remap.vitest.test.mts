...
describe("pixel-editor-skin-tone-remap", () => {
  it("remaps direct and ownership-assisted skin tone pixels", () => {
    ...
    assert.deepEqual(
      Array.from(
        remapSkinTonePixelsAny(unchangedPixels, [light.codePoint], []),
      ),
      Array.from(unchangedPixels),
    );
    assert.deepEqual(
    ...
    assert.deepEqual(
      Array.from(remappedEndpointDark),
      hexToRgba(EGA_COLORS[0]),
    );
