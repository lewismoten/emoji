...
describe("pixel-editor-layer-helpers", () => {
  it("covers layer transforms, compositing, palette quantization, and rotation", () => {
    ...
    try {
      ...
      assert.equal(
        layerPositionAllowedAny({ width: 3, height: 3 }, 0, 0),
        true,
      );
      assert.equal(
      ...
      const quantized = quantizeToPaletteAny(
        new Uint8ClampedArray([
          10, 10, 10, 254, 240, 240, 240, 255, 1, 2, 3, 0,
        ]),
        ["#000000", "#ffffff"],
      );
      assert.deepEqual(
      ...
      assert.deepEqual(
        Array.from(rotated.pixels.slice(0, 4)),
        [255, 255, 255, 255],
      );
      ...
      rotatedCanvasReuse.context.returnedData = new Uint8ClampedArray([
        0, 0, 0, 255,
      ]);
