...
const field = SequenceFilterFieldControl.create() as unknown as FakeElement;
assert.equal(
  field.className,
...
assert.equal((field.children[0] as FakeElement).className, "filter-heading");
assert.equal(
  ((field.children[0] as FakeElement).children[0] as FakeElement).getAttribute(
    "id",
  ),
  "sequence-filter-label",
);
assert.equal(
  ((field.children[1] as FakeElement).children[0] as FakeElement).textContent,
  "Not loaded",
);
assert.equal(
