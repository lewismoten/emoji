...
describe("newspeak-fallback-coverage", () => {
  it("keeps newspeak locale files complete enough to avoid English fallback", async () => {
    ...
    const blankStringKeys = (values: Record<string, string>) =>
      Object.entries(values)
        .filter(([, value]) => value.trim().length === 0)
        .map(([key]) => key)
        .sort();

    const blankAnnotationKeys = (values: Record<string, string[]>) =>
      Object.entries(values)
        .filter(
          ([, value]) =>
            value.length === 0 ||
