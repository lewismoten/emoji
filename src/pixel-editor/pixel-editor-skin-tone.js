...
export function skinToneCycle(codePoint) {
  const index = SKIN_TONE_COLORS.findIndex((tone) => tone.codePoint === codePoint);
  if (index < 0) return [];
  return [
    { kind: "normal", color: SKIN_TONE_COLORS[index].color },
    index > 0 ? { kind: "lighter", color: SKIN_TONE_COLORS[index - 1].color } : undefined,
...
export function skinToneSequence(codePoints = []) {
  ...
  return codePoints.map((codePoint) => String(codePoint).toUpperCase()).filter((codePoint) => skinTones.has(codePoint));
}
...
export function remapSkinTonePixels(pixels, sourceTones = [], targetTones = [], helper) {
  ...
  const pairColors = pairs.map(({ sourceTone, targetTone }) => skinToneColorMap(sourceTone, targetTone));
  const colors = new Map();
  for (const { sourceTone, targetTone } of pairs) {
    const source = findSkinTone(sourceTone);
    const target = findSkinTone(targetTone);
    if (source && target && !colors.has(source.color)) colors.set(source.color, target.color);
  }
  for (const { sourceTone, targetTone } of pairs) {
    for (const [sourceColor, targetColor] of skinToneColorMap(sourceTone, targetTone)) {
      if (!colors.has(sourceColor)) colors.set(sourceColor, targetColor);
    }
  }
  for (let offset = 0; offset < result.length; offset += 4) {
    if (result[offset + 3] === 0) continue;
    const sourceColor = rgbHex(result[offset], result[offset + 1], result[offset + 2]);
    const pixelIndex = offset / 4;
    const owner = helper?.ownership
      ? helper.ownership[(Math.floor(pixelIndex / helper.width) + (helper.offsetY ?? 0)) * helper.ownershipWidth + (pixelIndex % helper.width) + (helper.offsetX ?? 0)] ?? -1
      : -1;
    const targetColor = owner >= 0 ? pairColors[owner]?.get(sourceColor) : colors.get(sourceColor);
...
function skinToneColorMap(sourceTone, targetTone) {
  ...
  for (const sourceShade of skinToneCycle(sourceTone).filter((shade) => shade.kind !== "normal")) {
    const targetShade =
...
export function buildSkinToneOwnership(pixels, tones, width = CELL_SIZE, height = CELL_SIZE) {
  if (new Set(tones).size !== tones.length || tones.length < 2) return undefined;
  const normalOwners = new Map(tones.map((tone, index) => [findSkinTone(tone)?.color, index]));
  ...
  for (let index = 0; index < pixels.length / 4; index += 1) {
    ...
    const owner = normalOwners.get(rgbHex(pixels[offset], pixels[offset + 1], pixels[offset + 2]));
  ...
  for (let y = 0; y < height; y += 1) {
    for (let x = 0; x < width; x += 1) {
      ...
      for (const seed of seeds) {
        ...
        if (!nearest || distance < nearest.distance || (distance === nearest.distance && seed.owner < nearest.owner)) {
...
export function buildTwoPersonOwnership(width = CELL_SIZE, height = CELL_SIZE) {
  ...
  for (let y = 0; y < height; y += 1) {
    for (let x = 0; x < width; x += 1) ownership[y * width + x] = x < dividingColumn ? 0 : 1;
  }
  return ownership;
}

export function compareSkinToneHelpers(left, right) {
  const endpointTones = new Set([SKIN_TONE_COLORS[0].codePoint, SKIN_TONE_COLORS.at(-1).codePoint]);
  const endpointCount = (entry) => skinToneSequence(entry.codePoints).filter((tone) => endpointTones.has(tone)).length;
  return endpointCount(left) - endpointCount(right) || left.key.localeCompare(right.key);
}

function endpointSkinToneShade(codePoint, shadeKind) {
  if (codePoint === SKIN_TONE_COLORS.at(-1)?.codePoint && shadeKind === "darker") {
