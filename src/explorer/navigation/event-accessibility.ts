...
function getModifierGroupCheckboxes(checkbox: HTMLElement | null) {
  ...
  return Array.from(
    fieldset.querySelectorAll<HTMLInputElement>(
      "input.skin-tone, input.hair, input.gender",
    ),
  );
}
...
function onModifierKeyDown(event: KeyboardEvent) {
  ...
  if (event.key === "Home") nextIndex = 0;
    ...
    const columnIndex =
      rows[rowIndex]?.findIndex((item) => item.index === currentIndex) ?? -1;
    ...
    if (movePrevious || moveNext) {
      ...
      nextIndex =
        row[(columnIndex + offset + row.length) % row.length]?.index ??
        currentIndex;
    } else {
