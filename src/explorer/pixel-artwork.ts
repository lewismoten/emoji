export function createPixelArtworkManager(options: any) {
  ...
  const renderedPixelEmoji = (emojiKey: string) => {
    const value = options.emojiByKey()[emojiKey] ?? options.byId()[emojiKey]?.emoji ?? '';
    ...
    const sequenceLength = (options.byId()[emojiKey]?.codePoints ?? '')
      .split(/\s+/)
      .filter((point: string) => point && !['FE0E', 'FE0F'].includes(point.toUpperCase()))
      .length;
  ...
  const applyPixelArtworkClass = (element: HTMLElement | undefined, emojiKey: string) => {
    if (!element) return;
  ...
  const updatePixelArtworkManifest = (manifest: any) => {
    const glyphs = manifest.fields
      ? (manifest.glyphs ?? []).map((row: any[]) =>
          Object.fromEntries(
            manifest.fields.map((field: string, index: number) => [field, row[index]])
          )
        )
      : (manifest.glyphs ?? []);
    ...
    privateUseByKey = new Map(
      glyphs
        ...
        .map((glyph: any) => [glyph.key, Number.parseInt(glyph.privateUseCodePoint, 16)])
    );
  ...
  const refreshRenderedPixelEmoji = () => {
    document.querySelectorAll<HTMLElement>('[data-pixel-emoji-key]').forEach(element => {
