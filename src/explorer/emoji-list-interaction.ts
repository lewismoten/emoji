type EmojiCell = HTMLElement & { dataset: DOMStringMap & { emojiKey?: string } };
...
export function createEmojiListInteraction(options: {
  ...
  const yieldForListRender = () => {
    if ((window as any).scheduler?.yield) return (window as any).scheduler.yield();
  ...
  const createEmptyResults = () => {
    ...
    if (searchText.value.trim()) {
      ...
      clearSearch.textContent = options.translate('clearSearch', 'Clear search');
  ...
  const renderEmojiList = (keys: string[], shouldRestoreEmojiFocus: boolean) => {
    ...
    const renderChunk = () => {
      ...
      else finishEmojiListRender(generation, shouldRestoreEmojiFocus, renderRoot);
    };
    renderChunk();
  };
  ...
  const onEmojiFocus = (event: FocusEvent) => {
    ...
    options.emojiList().querySelectorAll<HTMLElement>('[data-emoji-key]').forEach(item => {
