export function upgradeEmojiDialog(options: {
  ...
  const dialogControls = options.exampleDialog.querySelector('.dialog-controls');
...
function upgradeEmojiPreview(dialog: HTMLElement) {
  ...
  let copyLabel = preview.querySelector<HTMLElement>('.emoji-preview-copy-label');
  if (!glyph || !copyLabel) {
...
function ensureRenderingDiagnostic(dialog: HTMLElement) {
  ...
  let invitation = dialog.querySelector<HTMLElement>('.pixel-design-invitation');
...
function ensureCodeDialogView(dialog: HTMLElement) {
  ...
  addAction(actions, 'show-pixel-editor developer-only', 'editPixelArt', 'Edit pixel art');
  ...
  actions?.querySelectorAll('[data-copy="key"], [data-copy="escape"], [data-copy="codePoints"], .show-emoji-code, .show-pixel-editor').forEach(element => element.classList.add('developer-only'));
  dialog.querySelectorAll('.rendering-diagnostic, .pixel-design-invitation, .emoji-composition, .emoji-metadata > div:has(.emoji-sequence-type), .emoji-metadata > div:has(.emoji-status)').forEach(element => element.classList.add('developer-only'));
}

function addAction(actions: Element | null, className: string, key: string, text: string) {
...
function addCopyAction(parent: Element, copy: string, key: string, text: string) {
...
function ensureCompactCopyLabels(dialog: HTMLElement) {
  ...
  dialog.querySelectorAll<HTMLElement>('[data-copy]:not(.emoji-preview)').forEach(button => {
