...
const inputs = (directory: string) =>
  Object.fromEntries(
    files(directory)
      .filter((file) =>
        file.endsWith(directory === sourceDirectory ? ".ts" : ".js"),
      )
      .map((file) => [
        path.relative(directory, file).replace(/\.[^.]+$/, ""),
        file,
      ]),
  );
...
const output = (format: "es" | "cjs", production: boolean): OutputOptions => ({
  ...
  entryFileNames: production
    ? `[name].min.${format === "es" ? "js" : "cjs"}`
    : `[name].${format === "es" ? "js" : "cjs"}`,
  chunkFileNames: production
    ? `[name]-[hash].min.${format === "es" ? "js" : "cjs"}`
