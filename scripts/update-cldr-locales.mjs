...
const requestedOrDefaultLocales =
  requestedLocales.length > 0 ? requestedLocales : defaultLocales;
...
const locales = [
  ...new Set(
    requestedOrDefaultLocales.flatMap((locale) => {
...
const characterLabelsFrom = (xml) => {
  const labels = {};
  const section =
    xml?.match(/<characterLabels>([\s\S]*?)<\/characterLabels>/)?.[1] ?? "";
  for (const match of section.matchAll(
    /<characterLabel type="([^"]+)"[^>]*>([\s\S]*?)<\/characterLabel>/g,
  )) {
...
const manifest = new Map(
  existingManifest.locales.map((locale) => [locale.locale, locale]),
);
...
for (const locale of locales) {
  ...
  const [
    ...
    baseLocale === locale
      ? null
      : fetchOptionalJson("cldr-annotations-full", "annotations", locale),
    baseLocale === locale
      ? null
      : fetchOptionalJson(
          "cldr-annotations-derived-full",
    ...
    baseLocale === locale
      ? null
      : fetchOptionalText(characterLabelsUrl(locale)),
  ]);
  const baseSource = {
  ...
  const annotationsToWrite =
    baseLocale === locale
      ? entries
      : Object.fromEntries(
          Object.entries(entries).filter(
            ([key, terms]) =>
              JSON.stringify(terms) !== JSON.stringify(baseEntries[key]),
          ),
        );
    ...
    baseLocale === locale
      ...
      : Object.fromEntries(
          Object.entries(labels).filter(
            ([key, label]) => label !== baseLabels[key],
          ),
        );
  const baseSubgroups = customSubgroupLabels[baseLocale] ?? {};
  const subgroups = {
    ...baseSubgroups,
    ...(customSubgroupLabels[locale] ?? {}),
  };
  const subgroupsToWrite =
    baseLocale === locale
      ? subgroups
      : Object.fromEntries(
          Object.entries(subgroups).filter(
            ([key, label]) => label !== baseSubgroups[key],
          ),
        );
  const file = `${locale}.json`;
  if (
    baseLocale !== locale &&
    Object.keys(annotationsToWrite).length === 0 &&
    Object.keys(labelsToWrite).length === 0 &&
    Object.keys(subgroupsToWrite).length === 0
  ) {
    fs.rmSync(path.join(outputDirectory, file), { force: true });
    manifest.delete(locale);
    console.info(
      `No ${locale}-specific CLDR data is available; omitted ${path.join(outputDirectory, file)} and use ${baseLocale} instead`,
    );
    continue;
  }
  fs.writeFileSync(
  ...
  console.info(
    `Wrote ${Object.keys(annotationsToWrite).length} CLDR annotations and ${Object.keys(labelsToWrite).length} character labels to ${path.join(outputDirectory, file)}`,
  );
}
...
fs.writeFileSync(
  ...
  `${JSON.stringify(
    {
      locales: [...manifest.values()].sort((left, right) =>
