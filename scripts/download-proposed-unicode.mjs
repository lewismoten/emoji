...
if (
  ...
  throw new Error(
    "Usage: npm run unicode:proposed -- [next|major.minor[.patch]] [--stage draft|beta] [--expected YYYY-MM]",
  );
}
const optionValue = (name) =>
  options
    .find((option) => option.startsWith(`${name}=`))
    ?.slice(name.length + 1);
...
const asKey = (text) => {
  ...
  key = key.replace(
    /^([A-Z]+)[A-Z][a-z]/,
    (match, prefix) => prefix.toLowerCase() + match.slice(-2),
  );
  key = key.replace(
    /[a-z][A-Z]([A-Z]+)$/,
    (match, suffix) => match.slice(0, 2) + suffix.toLowerCase(),
  );
...
const parseEmojiTest = (text) => {
  ...
  for (const line of text.split("\n")) {
    ...
    const [rawCodePoints, rawStatus] = line
      .split(";")
    ...
    emoji.push({
      emoji: String.fromCodePoint(
        ...rawCodePoints.split(" ").map((code) => Number.parseInt(code, 16)),
      ),
...
const draftVersion = draftText.match(
  /^# Version:\s*(\d+\.\d+(?:\.\d+)?)/m,
)?.[1];
if (!draftVersion)
  throw new Error(
    "Could not determine the Unicode version from the draft emoji data.",
  );
if (requestedVersion !== "next" && requestedVersion !== draftVersion) {
  throw new Error(
    `Requested Unicode ${requestedVersion}, but the draft data is Unicode ${draftVersion}.`,
  );
}
...
const duplicateKeys = candidates.filter(
  (item, index) => index > 0 && item.key === candidates[index - 1].key,
);
if (duplicateKeys.length) {
  throw new Error(
    `Duplicate proposed emoji keys: ${duplicateKeys.map((item) => item.key).join(", ")}`,
  );
}
...
const proposed = (manifest.proposed ?? []).filter(
...
fs.writeFileSync(
  manifestFile,
  `${JSON.stringify({ ...manifest, proposed }, null, 2)}\n`,
);
console.info(
  `Wrote ${candidates.length} proposed Unicode Emoji ${draftVersion} entries to ${outputFile}.`,
);
