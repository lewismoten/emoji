...
assert.match(demoStyles, /@media \(max-width: 560px\)[\s\S]*\.example-dialog > \.dialog-heading \{[\s\S]*grid-template-columns: minmax\(0, 1fr\) repeat\(2, 2rem\)/, 'mobile emoji-dialog headings must use a compact two-by-two control grid beside the title');
assert.match(demoStyles, /\.example-dialog \.emoji-previous \{ grid-row: 2;[\s\S]*\.example-dialog \.emoji-next \{ grid-row: 2;/, 'mobile emoji navigation must move below the complete title');
assert.doesNotMatch(demoStyles, /line-clamp/, 'emoji-dialog titles must not visually truncate long names');
assert.match(demoScript, /dialogTitleElement\.title = dialogTitle/, 'emoji-dialog titles must also expose the complete name as a tooltip');
...
assert.match(demoHtml, /class="dialog-title-row"[\s\S]*class="toggle-favorite"[\s\S]*id="example-title"/, 'the favorite star must appear beside the title by default');
...
assert.match(demoStyles, /\.modifier-filters fieldset label \{[\s\S]*min-height: 2\.25rem;[\s\S]*border: 1px solid var\(--border\)/, 'modifier filters must remain button-like on wide screens');
