...
const unicodeVersion =
  emojiVersion.split(".").length === 2 ? `${emojiVersion}.0` : emojiVersion;
...
const files = [
  ...
  [
    ...
    `emoji-variation-sequences-${emojiVersion}.txt`,
  ],
];
const releaseDates = {
  0.6: "2010-10-11",
...
const parseEmojiTest = (text) => {
  ...
  const duplicateKeys = emoji.filter(
    (item, index) => index > 0 && item.key === emoji[index - 1].key,
  );
  if (duplicateKeys.length)
    throw new Error(
      `Duplicate emoji keys: ${duplicateKeys.map((item) => item.key).join(", ")}`,
    );
...
for (const [version, keys] of [...versions.entries()].sort(([a], [b]) =>
  ...
  manifest.push({
    version,
    released: releaseDates[version] ?? null,
    file: `${version}.json`,
    count: keys.length,
  });
}
...
console.info(
  `Generated ${emoji.length} Unicode Emoji ${emojiVersion} entries.`,
);
