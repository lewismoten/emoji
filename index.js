...
const isInstalledApp = () => installedDisplayQueries.some(query => query.matches) ||
...
const isIosDevice = () => {
    ...
    if (clientPlatform?.toLowerCase() === 'macos')
        return false;
...
async function installApp(event) {
    ...
    if (!promptEvent) {
        ...
        const iosInstructions = installDialog?.querySelector('.install-instructions-ios');
        const browserInstructions = installDialog?.querySelector('.install-instructions-browser');
        if (iosInstructions)
            iosInstructions.hidden = !ios;
        if (browserInstructions)
            browserInstructions.hidden = ios;
...
function getExplorerSubGroup(item) {
    ...
    if (raw === 'country-flag')
        return getFlagContinent(item) ?? 'Other Flags';
    if (raw === 'animal-bug')
        return 'Bugs';
    if (raw === 'animal-bird')
        return 'Birds';
    if (raw === 'animal-mammal')
        return 'Mammals';
    if (raw === 'animal-marine')
        return 'Marine Animals';
    if (raw === 'animal-reptile')
        return 'Reptiles';
    if (raw === 'animal-amphibian')
        return 'Amphibians';
    if (raw === 'plant-flower')
        return 'Flowers';
    if (raw === 'plant-other')
    ...
    if (raw.startsWith('food-'))
        return titleCase(raw.slice(5));
    if (raw === 'clothing')
        return getClothingType(name);
    if (raw === 'geometric')
        return getGeometricShape(name);
    if (raw === 'family')
        return getFamilyType(name);
    if (raw === 'person')
        return /baby|boy|girl|child/.test(name) ? 'Children' : 'Adults';
    if (raw === 'person-role')
        return getPersonRoleType(name);
    if (raw === 'person-activity')
        return getActivityType(name);
    if (raw === 'person-fantasy')
        return getFantasyType(name);
    if (raw === 'person-gesture')
        return getGestureType(name);
    if (raw === 'person-sport')
        return getSportType(name);
...
function getClothingType(name) {
    if (/shoe|boot|sandal|slipper/.test(name))
        return 'Shoes';
    if (/cap|crown|helmet|hat|headscarf|hair pick/.test(name))
        return 'Hats & Headwear';
    if (/bag|purse|glasses|goggles|gloves|necktie|lipstick|fan|beads|gem/.test(name))
        return 'Accessories';
    return 'Clothing';
}
...
function getFamilyType(name) {
    if (name.startsWith('family'))
        return 'Families';
    if (name.startsWith('kiss'))
        return 'Kissing Couples';
    if (name.startsWith('couple with heart'))
        return 'Couples with Heart';
...
function getPersonRoleType(name) {
    ...
    if (/student|teacher|office worker/.test(name))
    ...
    if (/astronaut|pilot/.test(name))
        return 'Travel & Space';
    if (/prince|princess|crown/.test(name))
        return 'Royalty';
    return 'Cultural & Formal Wear';
}
function getActivityType(name) {
    if (/wheelchair|white cane/.test(name))
        return 'Accessibility & Mobility';
    if (/haircut|massage|steamy room/.test(name))
        return 'Personal Care & Rest';
    if (/dancing|bunny ears|ballet/.test(name))
        return 'Dance';
    if (/kneeling|standing|levitating/.test(name))
        return 'Poses';
    return 'Movement';
}
function getFantasyType(name) {
    if (/santa|claus|angel/.test(name))
        return 'Holiday & Angels';
    if (/elf|fairy|genie|mage/.test(name))
        return 'Magic';
    if (/superhero|supervillain/.test(name))
        return 'Heroes & Villains';
    if (/mermaid|merman|merperson/.test(name))
        return 'Merpeople';
    return 'Monsters & Undead';
}
function getGestureType(name) {
    if (name.startsWith('deaf'))
        return 'Accessibility';
    if (/raising hand|gesturing|tipping hand/.test(name))
        return 'Signals & Greetings';
    if (/facepalming|frowning|pouting|shrugging/.test(name))
        return 'Reactions';
    return 'Respect & Apology';
}
function getSportType(name) {
    if (/swimming|surfing|rowing|water polo/.test(name))
        return 'Water Sports';
    if (/biking|mountain biking/.test(name))
        return 'Cycling';
    if (/bouncing ball|handball|golfing/.test(name))
        return 'Ball Sports';
    if (/lifting weights|cartwheeling|juggling/.test(name))
        return 'Fitness & Skills';
    if (/ski|snowboard/.test(name))
        return 'Winter Sports';
    if (/wrestling|fencing|horse racing/.test(name))
        return 'Competition';
...
function ensureUtilityControls() {
    ...
    const helpLanguageControl = document.querySelector('.help-dialog .help-language-control');
...
function positionFavoriteButton() {
    ...
    const dialogControls = document.querySelector('.example-dialog .dialog-controls');
    if (!favoriteButton || !dialogTitleRow || !dialogControls)
        return;
...
function onPanelDialogClose(event) {
    if (suppressedPanelCloses.delete(event.currentTarget) ||
        !urlStateReady ||
        applyingUrlState)
        return;
...
function renderSavedEmojiList(container, empty, keys, source) {
    ...
    container.replaceChildren(...available.map((key, index) => {
        const button = document.createElement('button');
        button.type = 'button';
        ...
        button.setAttribute('aria-label', searchAnnotations[key]?.[0] ??
            byId[key]?.shortName ??
            displayEmojiKey(key));
        return button;
    }));
...
function ensureImportExamples() {
    ...
    if (!code || !importLine || !importString)
        return;
...
function updateEmojiComposition(item, value) {
    ...
    if (!section || !equation || !modeButton)
        return;
    const points = (item.codePoints ?? '')
        .split(/\s+/)
        .filter(Boolean)
    ...
    const displayedParts = compositionMode === 'full' || !canCondense
        ...
        : condensedParts.filter(part => !part.component || !isCondensedSequenceControl(part.component.point));
    ...
    equation.append(createCompositionTerm('=', createCompositionResult(value, item.shortName, item.key)));
}
...
function updateDialogNavigation() {
    ...
    const index = keys.indexOf(currentEmojiKey);
    if (emojiPrevious)
        emojiPrevious.disabled = index <= 0;
    if (emojiNext)
        emojiNext.disabled = index === -1 || index >= keys.length - 1;
    updateCompositionBackButton();
}
