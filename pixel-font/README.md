...
[Preview the deployed fonts](https://lewismoten.github.io/emoji/pixel-font/build/)
...
package. Compiled font files are published through the dedicated
`@lewismoten/pixel-emoji` package and versioned GitHub Releases.
...
Unicode coverage remains explicit in the generated manifest.
...
rather than separate Emoji 16-only or Emoji 17-only font families. Proposed
artwork remains in the separate Pixel Emoji Proposed family shipped alongside
the stable files.
...
Create a publishable package and versioned release files with:

```sh
npm run pixel-font:package
```

Font releases use tags such as `pixel-emoji-v1.0.0`. The dedicated workflow
verifies that the tag matches `fontVersion`, publishes
