...
for (const item of eligible) {
  const bucketKey = `${item.group}\0${item.subGroup}`;
...
for (const bucket of [...buckets.values()].sort(compareBuckets)) {
  ...
  for (let part = 0; part < partCount; part += 1) {
    ...
    const id = `${groupSlug}/${subGroupSlug}${suffix}`;
    const image = `${id}.png`;
...
const manifest = {
  ...
  url: config.url,
  createdDate: config.createdDate,
  kind: "grouped-subgroups-without-skin-or-hair-modifiers",
  layout: "grouped-subgroups-v1",
  cellSize: config.cellSize,
  cellPadding: config.cellPadding,
  slotSize,
  columns: config.columns,
...
console.log(
  `Mapped ${eligible.length.toLocaleString()} base emoji into ${manifestSheets.length} ` +
...
async function loadPreviousAssignments() {
  try {
    ...
    if (manifest.layout !== "grouped-subgroups-v1") return new Map();
